...
demo of four clean pairs would never show that a quarter of clusters have no local
detail to examine, and a broken pick-one screen would ship because no demo cluster was
ever ambiguous. The proportions below are the ones measured over all 1,807 clusters at
the live 0.48 cut — 40.1% ambiguous, 7.7% mixed resolution, 4.5% with no sharpness
signal, 0.5% large — scaled to 24 clusters.
...
different files for 62.2% of the real library. Until Task 10 they were the same file for
82 of 82 demo records, which silently disarmed every test written against the demo: the
mutation "serve the analysis raster instead of the display one" survived a full pass
because no fixture could tell the two apart (`the-demo-library-cannot-tell-the-two-
derivatives-apart`). They now differ for 58 of 82 (70.7%), and the 24 that do not are the
photos whose only local copy is already the small one — the real library's own shape,
not a simplification of it. The two copies of one photo share a colour and differ in
checker frequency, so which *photograph* is in a pane reads off the palette while which
*raster* it came from is legible in the pixels as well as in `naturalWidth`.
"""
...
def analysis_stripe(display_stripe: int) -> int:
    ...
    axis a magnification test (Task 12) works on.
    """
...
#:
#: **The rates are the measured ones, and they are not the Phase 1b plan's.** The plan
#: asked for ~13 ambiguous and ~7 mixed-resolution out of 24; both were derived from
#: figures since superseded — 13 is 52.7% of 24 (the ambiguity rate at the old 0.40 cut,
#: now 40.1% at 0.48) and 7 is 27.5% (the mixed-resolution estimate Task 2 measured down
#: to 7.7%). Reproducing them would build a demo that misrepresents the library by 30
#: percentage points on the single rate that decides how much of the review is a
#: pick-one screen. Actual: 10 ambiguous (41.7% vs 40.1%), 2 mixed-resolution (8.3% vs
#: 7.7%), 6 low-detail (25.0% vs 25.3%), 22 with criteria dropped (91.7% vs 90.3%).
#: The one deliberate over-representation is sharpness, at 2 clusters (8.3%) against a
#: measured 4.5%, so the plural case runs at all in a 24-cluster set.
_SPEC: tuple[tuple[int, bool, bool, tuple[str, ...], int, bool], ...] = (
    ...
    # only as pairs, where the challenger never changes at all, so a magnification that
    # placed a newly-loaded take using the previous take's dimensions passed everything
    # (Task 12's mutation M20). The real library has these: 6.6% of clusters mix the two
    # display classes and cluster sizes run to 30. The winner is position 2, so the small
    # raster at position 0 lands among the challengers rather than in the pinned keeper.
    # Swapped with the pair above rather than added, so the mixed-resolution rate, the
    # pair count and the 24-small-raster total are all exactly what they were.
...
def _cluster_from_spec(
    ...
    for position in range(size):
        ...
        # And so must the sub-scores, which is what Task 11 found they did not.
        # `sharpness` used to fall monotonically with position while `total` peaked at
        # `best`, so in 22 of these 24 clusters the winner was NOT the sharpest take and
        # the evidence panel explained every pick with the reason it lost. It also gave
        # no photo a sharpness of 1.0, which real data cannot do: `cluster_sharpness`
        # emits a ratio to the sharpest take, so exactly one member is always 1.00.
        #
        ...
        # 20-photo cluster 20x softer than its keeper, and the softest real group this
        # project has measured is 9.55x, with ordinary burst groups at 1.04-1.27x. The
        # softest take is therefore pinned: 1.35x for a confident cluster, and 1.02x for
        # an ambiguous one, so a close call looks close at every cluster size.
...
def demo_records(clusters: Sequence[Cluster]) -> dict[str, PhotoRecord]:
    """uuid -> record, for the image endpoint (Task 7) to resolve without a library."""
