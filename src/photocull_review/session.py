...
Everything else in the review UI binds to this shape, so two properties are structural
rather than incidental:

**It is built by reusing `calibrate.sample_to_dict`.** That function already carries a
fix that took real debugging effort to find — `Cluster.records` and `Cluster.scores`
are *not* index-aligned, because `rank_cluster` sorts the scores while the records keep
bucket order, so joining them by position gives every photo another photo's sub-scores.
It reads as entirely plausible and is wrong throughout. `build_session` enriches
...
HTTP to a browser, and the review UI fetches images by uuid instead. Stripping happens
here, in the builder every consumer goes through, rather than at serialisation time
later, where omitting it would be silent.
"""
...
#: back under. The resume check reads it.
SESSION_VERSION = 1
...
#: more detail than the calibration contact sheet already showed. A real library has a
#: meaningful share of clusters like this. The remedy is the explicit full-resolution
#: fetch, not magnification, so the owner needs to know which clusters they are before
#: spending time squinting at one.
...
#: Rare on a real library, and consequential enough to want flagging when it happens --
#: one accepted proposal on a large cluster stages nearly all of it.
LARGE_CLUSTER_SIZE = 10

#: Prose, never an icon. Each says what is weak *and* what to do about it, because "low
#: confidence" on its own gives the owner nowhere to go.
ANNOTATION_TEXT: dict[str, str] = {
    "ambiguous": (
        "A close call — the top two takes scored within the confidence margin of each "
        # Deliberately says no direction. This note used to sit between the image and
        # the numbers and got moved under the keeper's score table instead, after that
        # placement knocked the two panes' score rows out of line on the real share of
        # clusters that are close calls -- and a wording that had said "below" would
        # then have pointed at the wrong thing. Prose that names a position is prose
        # that a layout change silently falsifies.
...
class DisplayInfo:
    ...
    `local_path` is deliberately not spelled the obvious way: the project's standing
    iCloud-download guardrail check matches the attribute osxphotos uses for the
    original file, so a field named that here would report a false positive on every
    future run — and a safety check that cries wolf stops being read.
...
def cluster_key(cluster: Cluster) -> str:
    ...
    Order-independent on purpose. Bucket order is deterministic today, but this key is
    what a decision recorded on Monday is looked up by on Thursday, and the only thing
    that should invalidate it is the cluster genuinely containing different photos."""
...
def session_settings(cfg: ClusterConfig, source: str | Path | None = None) -> dict[str, Any]:
    ...
    Split out from `config_digest` because the resume check has to *name the field that
    moved*, and a digest is a one-way function. The decision log stores this dict beside
    the digest,
...
def digest_settings(settings: Mapping[str, Any]) -> str:
    ...
    because re-tuning the scorer changes only that nested dict."""
...
def config_digest(cfg: ClusterConfig, source: str | Path | None = None) -> str:
    ...
    The resume check refuses to resume a session when this moves, so it has to move for
    exactly the right reasons. Defined as the digest *of* `session_settings` rather than
    alongside it, so the stored snapshot and the stored digest cannot describe different
    configs."""
...
def annotations(cluster: Cluster, displays: dict[str, DisplayInfo]) -> list[dict[str, str]]:
    ...
    **The policy is a measurement.** Seven candidate signals were checked against a real
    library, and taking all of them would annotate the overwhelming majority of clusters
    — a badge on nearly every cluster says nothing at all, the same lesson that led
    unvalidated scoring signals to ship at zero weight. Two candidates were therefore
    demoted to the surface where they are actionable, and neither is discarded:

    - `dropped_criteria` is non-empty for the large majority of clusters (`horizon`
      alone accounts for most of that). That is the background condition of a real
      library, not a warning about one cluster. It stays a field on every entry and
      becomes the evidence panel — "ranked on sharpness and exposure" is useful prose;
      a warning triangle on nine clusters in ten is not.
    `is_ambiguous` was demoted the same way and has since been **promoted back**,
    because the surface it was demoted *to* no longer exists. The reasoning was
    "`proposed_marks` already leaves every take unset, so the UI shows a pick-one screen
    instead of a confirm screen" — true until every cluster started opening with a
    suggestion, which made every cluster a confirm screen. An ambiguous cluster is now
    byte-identical in shape to a confident one, so without this note a coin flip would
    present exactly as a considered recommendation. That is the silent-plausible-
    wrongness shape this project has hit more than once, and it is why the badge is not
    optional here.

    What is left annotates a meaningful minority of clusters on the attention signals,
    plus the share that are close calls — each for a reason the owner can act on.
    """
...
def proposed_marks(cluster: Cluster) -> dict[str, str]:
    ...
    A proposal, never a verdict — the propose-and-adjust model lets the owner flip any
    photo either way and end with several keepers.

    **Every cluster gets a suggestion, ambiguous ones included.** This supersedes an
    earlier design where an ambiguous cluster proposed nothing. The owner's instruction
    was explicit: the algorithm recommends, the UI decides. Withholding a suggestion
    from the real share of clusters that are near-ties made the tool least helpful
    exactly where the owner has the most work to do, and expressed low confidence as an
    *absence* — something the owner has to already understand in order to read. The
    near-tie is still surfaced, but as prose in `annotations`, where it can say what it
    means.
...
def build_session(
    ...
    *,
    ...
    Cost, measured over a real library: resolving every display raster is a real but
    modest one-time cost, paid once when a session opens and then read for days."""
