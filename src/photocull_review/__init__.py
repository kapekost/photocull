"""The Phase 1b review UI: a local web app, and the only place write-back lives.
...
- **`photoscript` is the write-back driver.** Its API surface includes
  `Album.remove()` and `PhotosLibrary.delete_album()` — the exact calls CLAUDE.md
  hard rule #1 forbids. Keeping it out of the core package means the code that
  *reads* a Photos library structurally cannot be the code that mutates it, and the
  reviewable blast radius of hard rule #1 is one directory rather than the tree.
